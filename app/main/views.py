...
def get_service():
    try:
        ...
        return render_template("edit_service.html", service_data=service_json), 200
    except KeyError:
...
def get_service_json(service_id):
    ...
    if access_token is None:
        print('Bearer token must be supplied in DM_API_BEARER environment variable')
        raise Exception("DM_API_BEARER token is not set")
    url = "https://api.digitalmarketplace.service.gov.uk/services/" + service_id
